...
public abstract class MixinNaturalSpawner {
    ...
    private static boolean mayMeRightDistanceToPlayerAndSpawnPoint(boolean original, ServerLevel level, ChunkAccess chunk, BlockPos.MutableBlockPos pos, double squaredDistance) {
        if (squaredDistance <= Math.pow(IntegerGamerules.MIN_SPAWN_DISTANCE.getValue(), 2)) {
            return false;
        } else if (level.getSharedSpawnPos().closerToCenterThan(new Vec3((double) pos.getX() + 0.5, pos.getY(), (double) pos.getZ() + 0.5), IntegerGamerules.MIN_SPAWN_DISTANCE.getValue())) {
            return false;
        } else {
